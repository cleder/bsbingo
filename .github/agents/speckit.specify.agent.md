...
d.
Run the script `.specify/scripts/bash/create-new-feature.sh --json "$ARGUMENTS"` with the calculated number and short-name:
...
5. List the failing items and specific issues
6. Update the spec to address each issue
7. Re-run validation until all items pass (max 3 iterations)
8. If still failing after 3 iterations, document remaining issues in checklist notes and warn user

   - **If [NEEDS CLARIFICATION] markers remain**:
9. Extract all [NEEDS CLARIFICATION: ...] markers from the spec
10. **LIMIT CHECK**: If more than 3 markers exist, keep only the 3 most critical (by scope/security/UX impact) and make informed guesses for the rest
11. For each clarification needed (max 3), present options to user in this format:
