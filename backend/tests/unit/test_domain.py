...
def test_select_random_words_draws_k_distinct_items_only_from_pool(pool, k):
    if len(pool) < k:
...
def test_has_bingo_true_for_any_complete_winning_line(line):
    assert has_bingo(frozenset(line)) is True


@given(marked=st.sets(st.integers(min_value=0, max_value=24), max_size=4))
def test_has_bingo_false_when_fewer_than_a_full_line_marked(marked):
