...
secrets:
	...
	kubectl config get-contexts --no-headers | \
		grep -E "(admin@)?bsbingo-$(ENVIRONMENT)" | \
		head -1 | \
		awk '{print $$2}' | \
	...
	envsubst < k8s/templates/secrets.yaml.template | op inject | if [ "$(DEBUG)" = "true" ]; then cat; else kubeseal -n bsbingo-$(ENVIRONMENT) --format yaml > k8s/$(ENVIRONMENT)/secrets.yaml; fi
